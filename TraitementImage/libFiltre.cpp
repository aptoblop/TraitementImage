...
{
   /* int grey=0;
	for (int i = 0; i < image.GetDataS(); i++) 
	{
    grey=(image.GetPixel(i)->GetRouge()+image.GetPixel(i)->GetVert()+image.GetPixel(i)->GetBleu())/3;
		if (grey < 127) { image.GetPixel(i)->SetAll(0); }
		if (grey >= 127) { image.GetPixel(i)->SetAll(255); }
	}*/
